...
<template>
  <div>
    ...
    <div flex flex-col justify-center items-center>
      ...
      <div p-5>正常拼音</div>
      <div flex flex-col justify-center items-center>
        <button v-if="piniynText.length > 0" class="m-3 text-sm btn" @click="handleCopy(1)"
                @keyup="handleCopy(1)">点击复制拼音</button>
        <div flex flex-row justify-center items-center ref="copyZimu">
          <p v-for="(item, index) in piniynText" :key="index" p="x-5">
            {{ item[0] }}
          </p>
        </div>
        <button v-if="hanziArr.length > 0" class="m-3 text-sm btn" @click="handleCopy(2)"
                @keyup="handleCopy(2)">点击复制汉字</button>
